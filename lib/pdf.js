...
const styles = StyleSheet.create({
  ...
  cellQty: { width: '7%', paddingLeft: 5, paddingRight: 5 },
  cellDesc: { width: '42%', paddingLeft: 5, paddingRight: 5 },
  cellPres: { width: '13%', paddingLeft: 5, paddingRight: 5 },
  cellPrice: { width: '12%', paddingLeft: 5, paddingRight: 5, textAlign: 'right' },
  cellDiscount: { width: '10%', paddingLeft: 5, paddingRight: 5, textAlign: 'right' },
  cellTotal: { width: '16%', paddingLeft: 5, paddingRight: 5, textAlign: 'right' },
  totalsContainer: {
    marginTop: 10,
    marginLeft: 'auto',
    width: '40%'
...
export function QuotePDF({ company, client, items, meta }) {
  ...
  const subtotal = items.reduce((acc, it) => {
    ...
    const lineDiscount = lineTotal * ((it.discount || 0) / 100);
    return acc + (lineTotal - lineDiscount);
  }, 0);

  // Aplicar descuento global
  const globalDiscount = meta.globalDiscount || 0;
  const subtotalAfterDiscount = subtotal - globalDiscount;

  const tax = +(subtotalAfterDiscount * meta.tax_rate).toFixed(2);
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.totalsContainer}>
          ...
          {globalDiscount > 0 && (
            <View style={styles.totalRow}>
              <Text>Descuento Global ({meta.discount?.toFixed(1)}%):</Text>
              <Text>-{meta.currency} {globalDiscount.toFixed(2)}</Text>
            </View>
          )}
          <View style={styles.totalRow}>
        ...
        <View style={styles.footer}>
          ...
          {company.bankAccounts && company.bankAccounts.length > 0 && (
            <View style={{ marginTop: 15, paddingTop: 15, borderTopWidth: 1, borderTopColor: '#e0e0e0' }}>
              <Text style={[styles.footerLabel, { marginBottom: 8, fontSize: 10 }]}>Datos Bancarios:</Text>
              {company.bankAccounts.map((account, idx) => (
                <View key={idx} style={{ marginBottom: 8, paddingLeft: 10 }}>
                  <Text style={[styles.footerText, { fontWeight: 'bold', marginBottom: 2 }]}>
                    • {account.bankName} ({account.currency})
                  </Text>
                  <Text style={[styles.footerText, { paddingLeft: 10, fontSize: 8 }]}>
                    Cuenta {account.accountType}: {account.accountNumber}
                  </Text>
                  <Text style={[styles.footerText, { paddingLeft: 10, fontSize: 8 }]}>
                    CCI: {account.cci}
                  </Text>
